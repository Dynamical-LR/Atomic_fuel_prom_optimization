...
def logshow(E, ovens):
    for x in range(E.shape[0]):
        print(ovens[x]['start_temp'], '\t\033[47m', end='')
    ...
    def valid_moves(self):
        ...
        for n, seria in enumerate(self.series):
            ...
            for x, oven in enumerate(self.ovens):
                ...
                lines = []
                for place_t in range(start - length):
                    new_line = line.copy()
...
if __name__=='__main__':
    data = json.load(open('train/day-50.json', 'r'))
    correct_json(data)

    ovens = data['ovens'][:Nn]
    serias = data['series'][:1000]
    game = OvenGame(ovens,serias,0)

    while(True):
        moves = game.valid_moves()
        if len(moves) == 0: break
        # game.make_move(moves[np.argmin([ (m[Is]!=5).sum() for m in moves])])
        # game.make_move(moves[np.argmax([ (m[Ip] * m[Is]!=5).sum() for m in moves])])
    ...
    f, n, m = game.metric()
    print()
    print(check(game.state(), ovens), '\tЗаполненность:', int(f*100), '   Кол-во серий', n, '   Средний номер серии:', int(m), '   Score:', game.score())
    print()
    logshow(game.state(), ovens)